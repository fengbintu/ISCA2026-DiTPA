...
try:
    ...
    XFORMERS_IS_AVAILBLE = False  # default is False, set to True to debug
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context):
        ...
        if XFORMERS_IS_AVAILBLE:  # the xformers lib allows less memory, faster training and inference
            ...
            attn = attn.softmax(dim=-1)  # [2,12,32,256]
...
class XBlock(nn.Module):
    ...
    def forward(self, x, context, language_embedding=None, return_attn=False):
        if self.use_checkpoint:
            return checkpoint(self._forward, x, context, language_embedding, return_attn)
        else:
            return self._forward(x, context, language_embedding, return_attn)
            
    def _forward(self, x, context, language_embedding=None, return_attn=False):
        y, _ = self.self_attn(self.norm1(x)) # x:[2,32,768], y:[2,32,768]
        x = x + self.drop_path(y)
        
        y, cross_attn = self.cross_attn(self.norm1_1(x), context) # context:[2,256,768]
        x = x + self.drop_path(y)
        ...
        if self.with_film:
            ...
            x = self.film_layer(x, language_embedding) # language_embedding: [2, 768]
...
class QFormer(nn.Module):
    ...
    def forward(self, context, language_embedding=None, num_queries=None, return_attn=False, **kwargs):
        B, L, C = context.shape
        ...
        attn_weights = []  # collect cross-attention weight in each layer
        for blk in self.blocks:
            if return_attn:
                x, attn = blk(x, context, language_embedding=language_embedding, return_attn=True)
                attn_weights.append(attn)  # collect cross-attention weight
            else:
