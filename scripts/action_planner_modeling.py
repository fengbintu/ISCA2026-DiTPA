...
class RobotTransformerNet(nn.Module):
    def __init__(
        ...
        self.total_vocab_size = 1 + self.vocab_size * 3 + 2 
    ...
    def forward(self, obs: dict, act: dict, obs_tokens=None, act_tokens=None, noisy_action_tokens=None, 
        ...
        if obs_tokens is None:
            if reused_context_image_tokens is None:
                if 'natural_language_embedding' in obs:
                    ...
                    context_image_tokens = self.image_tokenizer(image[:, :t-num_pred_action+1], context[:, :t-num_pred_action+1], wrist_image, depth_image) 
                else:
        ...
        ditpa_gparas.vlm_latency = ditpa_gparas.vlm_latency + (vlm_end_time - vlm_start_time)

        if not self.use_action_head_diff:
            if self.dim_align_type == 0:
                noisy_action_tokens = F.pad(noisy_action_tokens, (0,self.token_embedding_size -noisy_action_tokens.shape[-1]), mode='constant', value=0) # pad
            elif self.dim_align_type == 1:
                noisy_action_tokens = self.linear_dim_aligner(noisy_action_tokens) # linear
        ...
        if self.use_action_head_diff and noisy_action_tokens is not None and timesteps is not None:
            ...
            if self.use_action_head_diff == 4:
                full_tokens = torch.cat([context_image_tokens[:, :t-num_pred_action+1].flatten(1, 2), ], axis=-2)
            else:
                full_tokens = torch.cat([context_image_tokens[:, :t-num_pred_action+1].flatten(1, 2), null_input[:, :]], axis=-2)
            ...
            if self.use_action_head_diff in [2]: # token diffusion head
            ...
            if ditpa_gparas.modality_skip_flag == True:
                return ret_func(output_logits[:, -11:])
            else:
                output_logits = output_logits[:, context[:, 0].shape[1] :, :] # [1,76,7]
                if reg_token_nums > 0:
                    ...
                    return ret_func(output_logits[:, context_img_len+1:]) # [1,11,7]
